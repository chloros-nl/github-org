...
# Self-hosted runners are intentionally NOT managed here. The provider can only
# manage runner GROUPS (the paid access-scoping containers), not the runner
# agents themselves. On Free, register self-hosted runners into the org's
# built-in "Default" group out of band (`./config.sh --url ... --token ...`) —
# no Terraform resource exists for the runner agent. See the README.
